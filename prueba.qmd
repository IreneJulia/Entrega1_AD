...
summary_sd <- function(x) {
  resumen <- summary(x)
  resumen$sd <- sd(x)  # Agregar la desviación estándar
  return(resumen)
}


data.frame(c(summary(datos$bill_length_mm), summary(datos$bill_depth_mm)))

summary_sd(datos$bill_length_mm)
...
En este apartado queremos ver si, a nivel muestral, los pingüinos de diferentes islas presentas diferencias significativas.

```{r}
ping_Biscoe = datos[datos$island=="Biscoe",]
ping_Dream = datos[datos$island=="Dream",]
ping_Torgersen = datos[datos$island=="Torgersen",]
...
```{r}
mean(ping_Biscoe$bill_length_mm)
mean(ping_Dream$bill_length_mm)
mean(ping_Torgersen$bill_length_mm)
```
Notemos que los pinguinos de la isla `Torgersen` de media tienen el pico mas corto que los de las otras islas.
