...
par(mfrow=c(2,2))
pie(prop.table(table(datos$species)),col = c("lightpink","lightblue","purple"), main = "Proporción de especies")
pie(prop.table(table(datos$island)),col = c("yellow","orange","green"), main = "Proporción de cada isla")
pie(prop.table(table(datos$sex)), labels = c("Hembra", "Macho"), col = c("blue","coral"), main = "Sexo")
pie(prop.table(table(datos$year)),col = c("aquamarine","darksalmon","lightgreen"), main = "Año del experimento")
```

Observamos que hay el mismo número de machos que hembras, este hecho indica que seguramente se ha realizado un muestreo estratificado ya que la población ha sido dividida en dos estratos (machos y hembras).
